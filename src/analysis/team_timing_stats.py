...
# Garantir apenas colunas presentes
timing_cols = [c for c in timing_cols if c in df.columns]

timing_summary = df.groupby('teamname')[timing_cols].mean().reset_index()
timing_summary.to_csv('data/reports/team_timing_stats.csv', index=False)
print(timing_summary.head(10))
print("Relatório salvo em data/reports/team_timing_stats.csv")
